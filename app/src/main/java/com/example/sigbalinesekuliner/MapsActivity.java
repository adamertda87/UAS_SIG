...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private void getlokasi(){
        String url = "https://tipatcantok.000webhostapp.com/tipatcantok.php";
                        ...
                        for (int i = 0; i < jumdata; i++) {
                            try {
                                ...
                                mMap.addMarker(new MarkerOptions()
                                        .position(latLng[i])
                                        .snippet(jambuka[i])
                                        .icon(BitmapDescriptorFactory.fromResource(R.drawable.iconmapp))
                                        .title(nama[i]));
                                ...
                                for (int i = 0;i<jumdata; i++){
                                    if (marker.getTitle().equals(nama[i])) {
                                        if (MarkerD[i]) {
                                            ...
                                            Intent pindah = new Intent(MapsActivity.this, DetailActivity.class);
                                            startActivity(pindah);
                                            MarkerD[i] = false;
                                        } else {
                                            MarkerD[i] = true;
                                            marker.showInfoWindow();
                                            Toast pesan = Toast.makeText(MapsActivity.this, "Click lagi untuk membuka detai !", Toast.LENGTH_LONG);
